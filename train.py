...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--pos-intermediate-ctc-output",
        type=int,
        default=5,
        help="Which layer of the encoder will use the ctc phonetic auxiliary loss",
    )
...
def get_parser():
    ...
    parser.add_argument(
        "--phone-vocab",
        type=str,
        default="data/lang/phone.vocab",
        help="Path to the phone vocab file",
    )
    ...
    parser.add_argument(
        "--ctc-loss-scale-phones",
        type=float,
        default=0.3,
        help="Scale for CTC Phone loss.",
    )
...
def get_encoder_model(params: AttributeDict) -> nn.Module:
    ...
    encoder = Zipformer(
        ...
        pos_intermediate_ctc_output=params.pos_intermediate_ctc_output,
    )
    return encoder
...
def get_transducer_model(params: AttributeDict) -> nn.Module:
    ...
    model = Transducer(
        ...
        encoder_dim=int(params.encoder_dims.split(",")[-1]),
        intermediate_encoder_dim=int(
            params.encoder_dims.split(",")[params.pos_intermediate_ctc_output - 1]
        ),
        decoder_dim=params.decoder_dim,
        joiner_dim=params.joiner_dim,
        vocab_size=params.vocab_size,
        phone_vocab_size=params.phone_vocab_size,
    )
    return model
...
def compute_loss(
    ...
    if params.ctc_loss_scale_phones > 0:
        phones = batch["supervisions"]["extra"]
        phones = [''.join(x) for x in phones if x.strip()]
        token_ids_phones = vocab.encode(phones)

    with torch.set_grad_enabled(is_training):
        simple_loss, pruned_loss, ctc_output, ctc_output_phones = model(
            x=feature,
            x_lens=feature_lens,
            y=y,
    ...
    with warnings.catch_warnings():
        ...
        if params.ctc_loss_scale_phones > 0:
            assert ctc_loss_phones is not None
            supervision_segments_phones, token_ids_phones = encode_supervisions(
                supervisions,
                subsampling_factor=params.subsampling_factor,
                token_ids=token_ids_phones,
            )
    ...
    if params.ctc_loss_scale_phones > 0:
        assert ctc_output_phones is not None
        decoding_graph_phones = k2.ctc_graph(
            token_ids_phones, modified=False, device=device
        )
            
        dense_fsa_vec_phones = k2.DenseFsaVec(
            ctc_output_phones,
            supervision_segments_phones,
            allow_truncate=params.subsampling_factor - 1,
        )

        ctc_loss_phones = k2.ctc_loss(
            decoding_graph=decoding_graph_phones,
            dense_fsa_vec=dense_fsa_vec_phones,
            output_beam=params.beam_size,
            reduction="sum",
            use_double_scores=params.use_double_scores,
        )

        assert ctc_loss_phones.requires_grad == is_training

        loss += params.ctc_loss_scale_phones * ctc_loss_phones
    ...
    if params.ctc_loss_scale_phones > 0:
        info["ctc_loss_phones"] = ctc_loss_phones.detach().cpu().item()
...
def run(rank, world_size, args):
    ...
    if args.ctc_loss_scale_phones > 0:
        vocab = Vocab(params.phone_vocab)
        params.phone_vocab_size = len(vocab)
    else:
        params.phone_vocab_size = 0
    ...
    train_dataset = AsrDataModule(args)
    train_cuts = train_dataset.train_cuts("train_manifests")
    ...
    train_dl = train_dataset.train_dataloaders(
        train_cuts, sampler_state_dict=sampler_state_dict
    )

    valid_cuts = train_dataset.dev_cuts("dev_manifests")
    valid_dl = train_dataset.valid_dataloaders(valid_cuts)
