...
class Transducer(nn.Module):
    ...
    def __init__(
        ...
        if phone_vocab_size > 0:
            self.ctc_output_phone = nn.Sequential(
                nn.Dropout(p=0.1),
                nn.Linear(encoder_dim, phone_vocab_size),
                nn.LogSoftmax(dim=-1),
            )
        else:
            self.ctc_output_phone = None

        self.ctc_output = nn.Sequential(
            nn.Dropout(p=0.1),
            nn.Linear(intermediate_encoder_dim, vocab_size),
            nn.LogSoftmax(dim=-1),
        )
    ...
    def forward(
        ...
        encoder_out, encoder_out_intermediate, x_lens = self.encoder(x, x_lens)
        assert torch.all(x_lens > 0)

        # compute ctc log-probs
        ctc_output = self.ctc_output(encoder_out)
        if self.ctc_output_phone is not None:
            phone_ctc_output = self.ctc_output_phone(encoder_out_intermediate)
        else:
            phone_ctc_output = None
        ...
        return (simple_loss, pruned_loss, ctc_output, phone_ctc_output)
